...
{
	struct ls1x_nand_info *info = mtd->priv;
	struct nand_chip *chip =&info->nand_chip; //(struct nand_chip *)info;
	struct platform_device *pdev = info->pdev;
	uint64_t chipsize;
	int exit_nand_size;

	if(chip->cmdfunc==NULL){
		printk(KERN_WARNING "Filename:%s, Line:%d,Func:%s,flag:%d\n", __FILE__,__LINE__,__FUNCTION__,-1);
	}

	if (nand_scan_ident(mtd, 1, NULL))//error here,kernel dump!dstling 20220630
		return -ENODEV;
...
static void ls1x_nand_cmd_ctrl(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
...
static int ls1x_nand_probe(struct platform_device *pdev)
{
	...
	struct ls1x_nand_info *ls1xNandInfo;
	...
	/*
	mtd = devm_kzalloc(&pdev->dev, sizeof(struct mtd_info) + sizeof(struct ls1x_nand_info),GFP_KERNEL);
	if (!mtd)
		return -ENOMEM;
	ls1xNandInfo = (struct ls1x_nand_info *)(&mtd[1]);
	chip = (struct nand_chip *)(&mtd[1]);	// 注意指针指向
	*/

	//新内核对nand的数据结构要求 dstling add 20220630
	ls1xNandInfo = devm_kzalloc(&pdev->dev, sizeof(struct ls1x_nand_info),GFP_KERNEL);
	if (!ls1xNandInfo)
	{
		return -ENOMEM;
	}
	chip=&ls1xNandInfo->nand_chip;
	mtd=&chip->mtd;



	ls1xNandInfo->pdev = pdev;
	ls1xNandInfo->mtd = mtd;
	mtd->priv = ls1xNandInfo;
...
//	chip->ecc.mode		= NAND_ECC_SOFT_BCH;
	...
	chip->ecc.algo		= NAND_ECC_HAMMING;//dstling add 20220630
//	chip->controller	= &ls1xNandInfo->controller;
	...
	chip->cmd_ctrl		= ls1x_nand_cmd_ctrl;//dstling add 20220630
...
/*	ls1xNandInfo->clk = devm_clk_get(&pdev->dev, "apb_clk");
	if (IS_ERR(ls1xNandInfo->clk)) {
		dev_err(&pdev->dev, "failed to get nand clock\n");
		return PTR_ERR(ls1xNandInfo->clk);
	}
	ret = clk_prepare_enable(ls1xNandInfo->clk);
	if (ret < 0)
		return ret;*/
	...
	ls1xNandInfo->mmio_base = devm_ioremap_resource(&pdev->dev, r);
	if (IS_ERR(ls1xNandInfo->mmio_base)) {
		ret = PTR_ERR(ls1xNandInfo->mmio_base);
		goto fail_put_clk;
	}

	ret = ls1x_nand_init_buff(ls1xNandInfo);
	if (ret)
		goto fail_put_clk;

	/* initialize all interrupts to be disabled */
//	disable_int(ls1xNandInfo, NDSR_MASK);

	ret = request_irq(irq, ls1x_nand_irq, IRQF_DISABLED,
			  pdev->name, ls1xNandInfo);
	if (ret < 0) {
	...
	nand_gpio_init();
	ls1x_dma_init(ls1xNandInfo);
	ls1x_nand_init_hw(ls1xNandInfo);
//	chip->cmdfunc(mtd, NAND_CMD_RESET, 0, 0);
	platform_set_drvdata(pdev, ls1xNandInfo);
	...
	if (pdata->parts) {
		...
		ret = mtd_device_parse_register(ls1xNandInfo->mtd, part_probes, NULL,
		...
		//ppdata.of_node = pdev->dev.of_node;//dstling
		ret = mtd_device_parse_register(ls1xNandInfo->mtd, NULL, &ppdata,
						NULL, 0);
		return 0;
	}
...
fail_free_irq:
	free_irq(irq, ls1xNandInfo);
fail_free_buf:
	dma_free_coherent(&pdev->dev, ls1xNandInfo->dma_desc_size, ls1xNandInfo->dma_desc, ls1xNandInfo->dma_desc_phys);
	dma_free_coherent(&pdev->dev, ls1xNandInfo->data_buff_size, ls1xNandInfo->data_buff, ls1xNandInfo->data_buff_phys);
fail_put_clk:
//	clk_disable_unprepare(ls1xNandInfo->clk);
	return ret;
}
