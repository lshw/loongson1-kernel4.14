...
static void ls1x_spi_hw_init(struct ls1x_spi *hw)
{
	...
	writeb(0x03, hw->base + REG_SPER);//1字节触发中断  bit2 mode 1为标准模式 0为兼容模式
	//writeb(0xC3, hw->base + REG_SPER);//4字节触发中断 dstling
	//writeb(0x43, hw->base + REG_SPER);//2字节触发中断 dstling
#if defined(CONFIG_SPI_CS_USED_GPIO)
	...
	//printk("ls1x_spi_hw_init 1,REG_PARAM ret=%d\n",(int)val);//0001 1010
	val = val|0x0A; //dstling add
	//val &= 0xfe;//1111 1110  7：4  时钟分频数选择  bit3 dual IO, bit2 fast read, bit1 连续地址读 bit0 读使能
	val &= 0x0A;//1111 1110  7：4  时钟分频数选择  bit3 dual IO, bit2 fast read, bit1 连续地址读 bit0 读使能 dstling
	writeb(val, hw->base + REG_PARAM);
	//printk("ls1x_spi_hw_init 2,REG_PARAM ret=%d\n",(int)readb(hw->base + REG_PARAM));//0001 1010
	/* SPI flash时序控制寄存器 */
	writeb(0x05, hw->base + REG_TIMING);
	//writeb(0x04, hw->base + REG_TIMING);//dstling
}
...
static int ls1x_spi_probe(struct platform_device *pdev)
{
	...
	printk("ls1x_spi_probe hw->clk rate:%ld.\n",clk_get_rate(hw->clk));
